...
void vidOverlap() {
  if (keyCode == UP) {
    ...
    blendMode(LIGHTEST);
    image(src, 1200, height/3, width/3, height/3); //MIDDLE right
    blendMode(REPLACE);
    ...
    image(src, 0, 700, width/3, height/3);
    popStyle();
    ...
    image(src, width/3, 0, width/3, height/3); //top right
    ...
    blendMode(DARKEST);
    image(src, 1200, 700, width/3, height/3); //botom right
    image(src, 0, 300, width/3, height/3); //MIDDLE RIGHT
